  ...
  return [
    {
      ...
      message: spacingHint
        ...
        : "OpenAI API 키가 연결되지 않아 기본 예시 검사만 실행되었습니다.",
      suggestion:
        "실제 맞춤법과 띄어쓰기 검사를 사용하려면 OPENAI_API_KEY를 환경변수에 연결한 뒤 다시 검사하세요."
    },
    {
      type: "문장",
      message: longSentences.length > 0
        ? "긴 문장이 있어 주어와 서술어의 호응이 흐려질 수 있습니다."
        : "OpenAI API 키가 연결되지 않아 문장 비문 검사를 정밀하게 실행하지 못했습니다.",
      suggestion: longSentences.length > 0
        ? "한 문장에 여러 생각이 이어진 곳은 의미 단위별로 나누고, 주어와 서술어가 자연스럽게 맞는지 확인하세요."
        : "실제 문장 피드백을 받으려면 OPENAI_API_KEY를 환경변수에 연결한 뒤 다시 검사하세요."
    }
  ];
};
